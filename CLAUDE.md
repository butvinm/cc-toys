...
- README describes tools, not plugins: one H2 per plugin titled `name (plugin)` opening with the manifest description verbatim, then one H3 per tool titled `name (skill|command|hook)`. Skills and commands open with their trigger line, then a 1-3 line value-first description; hooks open with the value line, with the event and requirements last - a hook's event is mechanism, not value. optionally a one-line Try: and a collapsed example-output details block. Every clause is either a seller or a promise the shipped files back.
